...
export default function NotFound() {
  ...
  return (
    <div className="mx-auto max-w-[700px] px-4 py-28 text-center sm:px-6">
      ...
      <p className="mx-auto mt-4 max-w-sm font-body text-[12px] leading-relaxed text-muted">
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-2">
        ...
        {CATEGORIES.map((cat) => (
          <Link
            key={cat.id}
            to={href(`/shop/${cat.id}`)}
            className="rounded-full border border-ink/18 px-5 py-2.5 font-body text-[11px] uppercase text-muted transition-colors hover:border-ink/40 hover:text-ink"
          >
            {cat.label}
          </Link>
        ))}
