    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-24">
        ...
        <div className="mt-6 text-center">
          <Link to={href('/orders')} className="font-body text-[14px] uppercase text-accent underline underline-offset-4">
  ...
  return (
    <div className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6">
      ...
      <div className="text-center">
        ...
        <p className="mt-3 font-body text-[14px] uppercase text-muted">
          {order.number} · {placed.toLocaleDateString(config.locale, { dateStyle: 'long' })}
        </p>
        <p className="mx-auto mt-4 max-w-md font-body text-[14px] leading-relaxed text-muted">
      ...
      <div className="mt-10 rounded-[18px] border border-accent/30 bg-accent/[0.05] p-5">
        <h2 className="mb-2 font-grotesk text-[15px] uppercase text-accent">What happens next</h2>
        <p className="font-body text-[14px] leading-relaxed text-ink/80">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="rounded-[20px] border border-ink/10 bg-ink/[0.02] p-6">
          <h2 className="mb-5 font-grotesk text-[15px] uppercase text-ink">
            ...
            {order.lines.map((line) => (
              <li key={line.sku} className="flex items-start justify-between gap-4 py-3">
                <div className="min-w-0">
                  <div className="font-grotesk text-[14px] uppercase text-ink">{line.productName}</div>
        ...
        <aside>
          <div className="rounded-[20px] border border-ink/10 bg-ink/[0.02] p-6">
            <h2 className="mb-5 font-grotesk text-[15px] uppercase text-ink">Total paid</h2>
            <TotalsRows totals={order.totals} />
            {order.promoCode && (
              <p className="mt-3 font-body text-[12px] uppercase text-accent">Code {order.promoCode} applied</p>
            )}
            {order.paymentInstructions && (
              <p className="mt-3 font-body text-[12px] leading-relaxed text-amber-200/80">
                {order.paymentInstructions}
              </p>
            )}
          ...
          <Link
            to={href('/shop')}
            className="mt-4 block rounded-full border border-ink/20 py-3 text-center font-grotesk text-[14px] uppercase text-ink transition-colors hover:border-ink/45"
          >
            Continue shopping
          </Link>
        </aside>
...
function Block({
  ...
  return (
    <div>
      <div className="mb-2 flex items-center gap-2 font-body text-[12px] uppercase tracking-wide text-muted">
        <Icon size={12} /> {title}
      </div>
      <div className="font-body text-[13px] leading-relaxed text-ink/80">{children}</div>
    </div>
  )
}
