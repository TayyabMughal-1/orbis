...
const field =
  'w-full rounded-[12px] border border-ink/15 bg-background px-4 py-3 font-body text-[15px] text-ink outline-none transition-colors focus:border-accent'
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-[1100px] px-4 py-16 sm:px-6 lg:px-10">
        {checking ? (
          <p className="font-body text-[14px] text-muted">Checking your session…</p>
        ) : customer ? (
          <SignedIn
            customer={customer}
            onSignOut={() => {
  ...
  return (
    <div className="mx-auto max-w-[440px]">
      ...
      <form onSubmit={submit} className="mt-8 grid gap-3">
        ...
        {error && (
          <p className="rounded-[10px] border border-red-500/25 bg-red-50 p-3 font-body text-[13px] leading-relaxed text-red-700">
            {error}
          </p>
        )}
        ...
        <button
          type="submit"
          disabled={busy}
          className="press mt-1 rounded-full bg-ink py-3.5 font-grotesk text-[15px] uppercase text-background disabled:opacity-40"
        >
      ...
      <button
        ...
        className="mt-5 font-body text-[13px] text-muted underline hover:text-ink"
      >
    ...
    <>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          {customer.name && (
            <p className="mt-1 font-body text-[14px] text-muted">{customer.email}</p>
          )}
        </div>
        <button
          type="button"
          onClick={onSignOut}
          className="press inline-flex items-center gap-2 rounded-full border border-ink/20 px-5 py-2.5 font-grotesk text-[14px] uppercase text-ink transition-colors hover:border-ink/45"
        >
      ...
      {orders.loading && <p className="mt-4 font-body text-[14px] text-muted">Loading…</p>}
      {orders.error && (
        <div className="mt-4">
      ...
      {orders.data && orders.data.length === 0 && (
        <div className="mt-6 rounded-[18px] border border-ink/[0.07] bg-surface p-8 text-center">
          ...
          <Link
            to={shopHref}
            className="press mt-5 inline-block rounded-full bg-neon px-6 py-2.5 font-grotesk text-[14px] uppercase text-ink"
          >
            Start shopping
          </Link>
        </div>
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="mt-1 font-body text-[12px] uppercase tracking-wide text-muted">
                      {order.status}
                    </div>
                  </div>
                </div>
                <div className="mt-3 truncate font-body text-[13px] text-muted">
