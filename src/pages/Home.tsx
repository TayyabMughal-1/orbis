    ...
    <>
      ...
      <section className="relative min-h-[86vh] w-full overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex min-h-[86vh] max-w-[1600px] flex-col justify-center px-4 py-24 sm:px-6 lg:px-10">
          <div className="max-w-[820px]">
            <div className="animate-fade-up mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.15em] text-cream/80">
              <span aria-hidden="true">{config.flag}</span>
              {config.hero.eyebrow}
            </div>

            <h1
              className="animate-fade-up relative font-grotesk text-[42px] uppercase leading-[1.02] text-cream sm:text-[64px] md:text-[78px] lg:text-[92px]"
              style={{ animationDelay: '90ms' }}
            >
              Meet Orbis
              <br />
              take him home
              <span className="absolute -bottom-7 right-0 -rotate-1 font-condiment text-[26px] normal-case text-neon opacity-90 mix-blend-exclusion sm:-bottom-11 sm:text-[40px] lg:-bottom-14 lg:text-[52px]">
                small runs only
              </span>
            </h1>

            <p
              className="animate-fade-up mt-16 max-w-md font-mono text-[13px] uppercase leading-relaxed text-cream/70 sm:mt-20 lg:mt-24"
            ...
            <div className="animate-fade-up mt-9 flex flex-wrap items-center gap-3" style={{ animationDelay: '310ms' }}>
              <Link
                to={href('/shop')}
                className="press group flex items-center gap-2 rounded-full bg-neon px-7 py-3.5 font-grotesk text-[13px] uppercase text-background transition-opacity hover:opacity-90"
              >
                Shop everything
              ...
              <Link
                ...
                className="press rounded-full border border-white/25 px-7 py-3.5 font-grotesk text-[13px] uppercase text-cream transition-colors hover:border-white/50"
              >
                Shop the figures
              </Link>
            </div>
        ...
        <div className="mx-auto grid max-w-[1600px] grid-cols-2 gap-px px-4 sm:px-6 lg:grid-cols-4 lg:px-10">
          {[
            ...
            <Reveal key={title} delay={i * 80} className="px-2 py-8 sm:px-4 lg:px-6">
              <Icon size={18} className="mb-3 text-neon" strokeWidth={1.6} />
              <div className="font-grotesk text-[12px] uppercase text-cream">{title}</div>
              <p className="mt-1.5 font-mono text-[11px] leading-relaxed text-cream/50">{body}</p>
            </Reveal>
          ))}
        </div>
      </section>
          ...
          title={
            <>
              ...
              <span className="ml-10 sm:ml-20">
                are <span className="font-condiment normal-case text-neon">actually</span> buying
              </span>
            </>
          }
        >
          <Link
            to={href('/shop')}
            className="group flex items-center gap-2 font-grotesk text-[13px] uppercase text-cream hover:text-neon"
          >
            See everything
      ...
      <section className="mx-auto max-w-[1600px] px-4 pb-24 sm:px-6 lg:px-10">
        <CategoryCarousel
          intro={
            <div>
              <div className="mb-3 font-mono text-[11px] uppercase tracking-[0.2em] text-cream/50">
                Browse by type
              </div>
              <h2 className="font-grotesk text-[30px] uppercase leading-[1.05] text-cream sm:text-[40px] lg:text-[52px]">
                Departments
              </h2>
              <p className="mt-5 max-w-[32rem] font-mono text-[12px] leading-relaxed text-cream/55">
              ...
              <Link
                to={href('/shop')}
                className="group mt-6 inline-flex items-center gap-2 font-grotesk text-[13px] uppercase text-cream hover:text-neon"
              >
                Shop everything
                <ArrowRight size={15} className="transition-transform group-hover:translate-x-1" />
              </Link>
        ...
        <div className="mx-auto max-w-[1600px] px-4 py-16 sm:px-6 lg:px-10">
          <div className="mb-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="font-grotesk text-[26px] uppercase leading-none text-cream sm:text-[34px]">
          ...
          <RevealGroup className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              ...
              <figure
                key={review.name}
                className="flex h-full flex-col rounded-[18px] border border-white/10 bg-white/[0.02] p-6"
              >
                <Stars
                  ...
                  className="mb-4"
                />
                <blockquote className="flex-1 font-mono text-[12px] leading-relaxed text-cream/70">
                  “{review.quote}”
                </blockquote>
                <figcaption className="mt-5 font-mono text-[11px] uppercase text-cream/45">
                  {review.name} · <span className="text-neon/70">{review.place}</span>
                </figcaption>
              </figure>
            ))}
          </RevealGroup>
      ...
      <section className="relative w-full overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-[1600px] px-4 py-28 sm:px-6 lg:px-10">
          ...
          <span className="pointer-events-none absolute right-4 top-20 rotate-2 text-right font-condiment text-[30px] normal-case leading-[0.85] text-neon opacity-90 mix-blend-exclusion sm:right-6 sm:top-24 sm:text-[52px] lg:right-10 lg:top-28 lg:text-[64px]">
            since
            <br />
            2020
          </span>
          ...
          <Reveal className="max-w-[640px]">
            <h2 className="font-grotesk text-[32px] uppercase leading-[1.05] text-cream sm:text-[46px] lg:text-[58px]">
              Hello!
              <br />
              I&apos;m <span className="text-neon">orbis</span>
            </h2>
            <p className="mt-12 font-mono text-[13px] uppercase leading-relaxed text-cream/70">
            ...
            <p className="mt-5 max-w-md font-mono text-[12px] leading-relaxed text-cream/45">
      ...
      <section className="relative w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center bg-background/35">
          <div className="mx-auto w-full max-w-[1600px] px-4 text-right sm:px-6 lg:px-10">
            <h2 className="font-grotesk text-[18px] uppercase leading-[1.15] text-cream sm:text-[30px] md:text-[44px] lg:text-[58px]">
            ...
            <Link
              to={href('/shop/figures')}
              className="press mt-6 inline-flex items-center gap-2 rounded-full bg-neon px-6 py-3 font-grotesk text-[12px] uppercase text-background sm:mt-10"
            >
              Shop the figures
              <ArrowRight size={15} />
            </Link>
