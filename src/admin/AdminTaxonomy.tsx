...
const input =
  'w-full rounded-[10px] border border-ink/15 bg-background px-3 py-2 font-mono text-[12px] text-ink outline-none transition-colors focus:border-accent'
...
function Panel({
  ...
  return (
    <section className="rounded-[20px] border border-ink/[0.07] bg-background p-6 shadow-card">
      ...
      <p className="mt-2 max-w-[46ch] font-mono text-[11px] leading-relaxed text-muted">{blurb}</p>
      ...
      {error && (
        <p className="mt-4 rounded-[10px] border border-red-500/25 bg-red-50 p-3 font-mono text-[11px] leading-relaxed text-red-700">
          {error}
        </p>
      )}

      <div className="mt-6">
        {rows.loading && <p className="font-mono text-[11px] text-muted">Loading…</p>}
        {rows.error && <ErrorState message={rows.error} onRetry={rows.reload} />}
        {rows.data && rows.data.length === 0 && (
          <p className="font-mono text-[11px] text-muted">{emptyHint}</p>
        )}
        {rows.data && rows.data.length > 0 && (
              ...
              <li key={row.id} className="flex items-start gap-3 py-3">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {!row.active && (
                      <span className="rounded-full bg-ink/10 px-2 py-0.5 font-mono text-[9px] uppercase text-muted">
                        paused
                      </span>
                    )}
                  </div>
                ...
                <span className="whitespace-nowrap pt-0.5 font-mono text-[10px] text-muted">
