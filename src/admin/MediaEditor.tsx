    ...
    <div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {media.map((item, i) => (
          ...
          >
            ...
            {i === 0 && (
              <span className="absolute left-2 top-2 rounded-full bg-ink px-2 py-0.5 font-body text-[9px] uppercase tracking-wide text-background">
                Main
              </span>
            )}

            <div className="flex items-center justify-between border-t border-ink/[0.07] px-2 py-1.5">
              <span className="font-body text-[9px] uppercase tracking-wide text-muted">
                {item.kind}
              </span>
        ...
        {media.length === 0 && (
          <p className="col-span-full font-body text-[11px] text-muted">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="press inline-flex items-center gap-2 rounded-full bg-ink px-4 py-2 font-grotesk text-[11px] uppercase text-background disabled:opacity-40"
        >
          <Upload size={13} />
        ...
        <div className="flex min-w-[240px] flex-1 items-center gap-2 rounded-full border border-ink/15 bg-background px-3">
          ...
          <input
            ...
            className="w-full bg-transparent py-2 font-body text-[12px] text-ink outline-none placeholder:text-muted"
          />
          <button
            type="button"
            onClick={() => {
            ...
            disabled={!url.trim()}
            className="flex-none font-grotesk text-[11px] uppercase text-accent disabled:opacity-40"
          >
            Add
          </button>
        </div>
      ...
      {error && (
        <p className="mt-3 rounded-[10px] border border-red-500/25 bg-red-50 p-3 font-body text-[11px] leading-relaxed text-red-700">
          {error}
        </p>
      )}
    </div>
