      ...
      <div className="mx-auto max-w-[1600px] px-4 py-14 sm:px-6 lg:px-10">
        <div className="grid grid-cols-2 gap-10 lg:grid-cols-5">
          <div className="col-span-2 lg:col-span-2">
            ...
            <p className="mt-3 max-w-xs font-body text-[14px] leading-relaxed text-muted">
            ...
            <div className="mt-6 space-y-2">
              <a
                ...
                className="flex items-center gap-2 font-body text-[14px] text-ink/80 hover:text-accent"
              >
              ...
              <div className="font-body text-[13px] text-muted">{config.support.hours}</div>
            </div>
          </div>

          <div>
            <h3 className="font-grotesk text-[14px] uppercase tracking-wide text-ink">Shop</h3>
            <ul className="mt-4 space-y-2">
              <li>
                <Link to={href('/shop')} className="font-body text-[14px] text-muted hover:text-accent">
                  All products
                </Link>
              </li>
              {CATEGORIES.map((cat) => (
                <li key={cat.id}>
                  <Link
                    to={href(`/shop/${cat.id}`)}
                    className="font-body text-[14px] text-muted hover:text-accent"
                  >
                    {cat.label}
                  </Link>
                </li>
          ...
          <div>
            <h3 className="font-grotesk text-[14px] uppercase tracking-wide text-ink">
              Delivery &amp; returns
            </h3>
            <ul className="mt-4 space-y-2 font-body text-[14px] text-muted">
          ...
          <div>
            <h3 className="font-grotesk text-[14px] uppercase tracking-wide text-ink">We accept</h3>
            <ul className="mt-4 space-y-2">
              {config.paymentMethods.map((method) => (
                <li key={method.id} className="font-body text-[14px] text-muted">
                  {method.label}
                </li>
              ))}
            </ul>
        ...
        <div className="mt-12 flex flex-wrap items-center gap-2 border-t border-ink/10 pt-8">
          <span className="mr-2 font-body text-[12px] uppercase tracking-[0.2em] text-muted">
            Other stores
          </span>
          {REGION_LIST.map((r) => (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 font-body text-[13px] uppercase transition-colors ${
                r.code === region
        ...
        <div className="mt-8 flex flex-col gap-2 font-body text-[13px] leading-relaxed text-muted sm:flex-row sm:items-end sm:justify-between">
          <div>
