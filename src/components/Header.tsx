...
export default function Header() {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `font-grotesk text-[14px] uppercase tracking-wide transition-colors ${
  ...
  return (
    <>
      ...
      <div className="relative z-[70] bg-accent/90 px-4 py-2 text-center font-body text-[12px] uppercase tracking-[0.12em] text-ink">
        {config.hero.promo}
      ...
      >
        <div className="mx-auto flex max-w-[1600px] items-center gap-4 px-4 py-4 sm:px-6 lg:px-10">
          ...
          >
            ...
            <span className="ml-1.5 font-body text-[12px] tracking-widest text-muted">
              {config.countryCode}
            </span>
          </Link>
          ...
          <div className="ml-auto flex items-center gap-2 sm:gap-3">
            <form onSubmit={submitSearch} className="hidden items-center md:flex">
              ...
              >
                ...
                <input
                  ...
                  className="w-full bg-transparent py-2 pl-2 font-body text-[14px] text-ink outline-none placeholder:text-muted"
                />
              </div>
            </form>
            ...
            >
              ...
              {count > 0 && (
                ...
                <span
                  key={count}
                  className="animate-cart-pop absolute -right-1 -top-1 flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-neon px-1 font-body text-[12px] font-bold text-ink"
                >
                  {count}
                </span>
              )}
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[80] bg-background lg:hidden">
          ...
          <div className="px-4 py-6">
            <div className="mb-3 font-body text-[12px] uppercase tracking-[0.2em] text-muted">Store</div>
            <RegionSwitcher />
          </div>
        </div>
      )}
