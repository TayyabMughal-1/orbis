...
export function TotalsRows({ totals, shippingPending }: { totals: OrderTotals; shippingPending?: boolean }) {
  ...
  return (
    <dl className="space-y-2.5 font-body text-[14px]">
      ...
      <div className="border-t border-ink/10 pt-3">
        ...
        {totals.taxIncluded > 0 && (
          <p className="mt-1.5 text-right font-body text-[12px] uppercase text-muted">
    ...
    return (
      <div className="flex items-center gap-2 rounded-[12px] border border-accent/35 bg-accent/[0.07] px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="font-body text-[13px] uppercase text-accent">{code}</div>
          <div className="truncate font-body text-[12px] text-muted">{appliedLabel}</div>
        </div>
        <button
          type="button"
          onClick={onClear}
    ...
    <form onSubmit={submit}>
      <div className="flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-[12px] border border-ink/15 bg-ink/5 px-3 py-2.5 font-body text-[14px] uppercase text-ink outline-none placeholder:text-muted focus:border-ink/40"
        />
        <button
          type="submit"
          disabled={busy || !draft.trim()}
          className="flex-none rounded-[12px] border border-ink/20 px-4 py-2.5 font-body text-[13px] uppercase text-ink transition-colors hover:border-ink/45 disabled:opacity-35"
        >
          {busy ? '…' : 'Apply'}
        </button>
      </div>
      {error && <p className="mt-2 font-body text-[12px] text-amber-300">{error}</p>}
    </form>
  )
}
