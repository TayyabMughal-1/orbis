      ...
      >
        <div className="flex items-center justify-between border-b border-ink/10 px-5 py-4">
          <div>
            ...
            <span className="ml-2 font-body text-[11px] text-muted">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {resolved.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <p className="font-body text-[12px] uppercase leading-relaxed text-muted">
                Your cart is empty.
              </p>
              <Link
                to={href('/shop')}
                onClick={close}
                className="mt-5 rounded-full bg-neon px-6 py-2.5 font-grotesk text-[12px] uppercase text-ink"
              >
                Start shopping
              </Link>
            </div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <Link
                      to={href(`/product/${product.slug}`)}
                      onClick={close}
                      className="block truncate font-grotesk text-[12px] uppercase text-ink hover:text-accent"
                    >
                      {product.name}
                    </Link>
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        className="font-body text-[10px] uppercase text-muted underline underline-offset-2 hover:text-ink"
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  <div className="flex-none text-right">
                    <Price amount={lineTotal} className="font-body text-[13px] text-accent" />
                    {line.quantity > 1 && (
        ...
        {resolved.length > 0 && (
          <div className="border-t border-ink/10 px-5 py-4">
            ...
            <p className="mt-1 font-body text-[10px] uppercase text-muted">
            ...
            <Link
              to={href('/checkout')}
              onClick={close}
              className="press mt-4 block rounded-full bg-neon py-3 text-center font-grotesk text-[13px] uppercase text-ink transition-opacity hover:opacity-90"
            >
              Checkout
            </Link>
            <Link
              to={href('/cart')}
              onClick={close}
              className="mt-2 block py-2 text-center font-body text-[11px] uppercase text-muted hover:text-ink"
            >
              View full cart
            </Link>
          </div>
