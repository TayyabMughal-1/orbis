...
export default function Newsletter() {
  ...
  if (state === 'done') {
    return (
      <div className="flex items-start gap-2.5 rounded-[14px] border border-neon/30 bg-neon/[0.06] p-4">
        <Check size={15} className="mt-0.5 flex-none text-neon" strokeWidth={2.5} />
        <p className="font-mono text-[11px] leading-relaxed text-cream/70">{message}</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={onSubmit} noValidate>
      <label htmlFor="newsletter" className="block font-grotesk text-[12px] uppercase text-cream">
        10% off your first order
      </label>
      <p className="mt-1.5 font-mono text-[11px] leading-relaxed text-cream/50">
        New drops, restocks and the odd discount. One email a month at most, and you can leave
        whenever you like.
      </p>

      <div className="mt-3 flex gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3">
          <Mail size={14} className="flex-none text-cream/45" />
          <input
            id="newsletter"
            type="email"
            value={email}
            ...
            autoComplete="email"
            className="w-full bg-transparent py-2.5 font-mono text-[12px] text-cream outline-none placeholder:text-cream/30"
          />
        </div>
        <button
          type="submit"
          disabled={state === 'busy'}
          className="press flex-none rounded-full bg-neon px-5 py-2.5 font-grotesk text-[11px] uppercase text-background transition-opacity hover:opacity-90 disabled:opacity-50"
        >
          {state === 'busy' ? '…' : 'Join'}
        </button>
      </div>
