...
async function main() {
  try {
    ...
    if (!process.env.MONGODB_URI) {
      ...
      console.error('[api] Add it to .env — see .env.example for how to get one from')
      console.error('[api] MongoDB Atlas (the free tier is plenty):\n')
      console.error('[api]   MONGODB_URI=mongodb+srv://user:password@cluster0.xxxxx.mongodb.net/\n')
      process.exit(1)
    }
    console.error('\n[api] Could not connect to MongoDB.')
    console.error('[api] ' + (err instanceof Error ? err.message : String(err)))
    console.error('[api] Check the username and password in MONGODB_URI, and that your IP')
